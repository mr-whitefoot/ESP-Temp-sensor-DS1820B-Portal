  ...
  //Preferences
    ...
    GP.FORM_BEGIN(form.preferences);
      ...
      GP.BLOCK_TAB_BEGIN("Device name");
        GP.TEXT("tempLabel", "Temperature label", data.tempLabel); GP.BREAK();
      ...
      GP.BLOCK_TAB_BEGIN("Settings");
        ...
        GP.BOX_END();
        GP.NUMBER("sensorRefreshTime", "Sensor refresh time", data.sensorRefreshTime);  GP.BREAK();
      GP.BLOCK_END();
    ...
    GP.PAGE_TITLE(data.tempLabel);
    GP.FORM_BEGIN(form.root);
       GP.BLOCK_TAB_BEGIN("Sensor");
          GP.BOX_BEGIN(GP_EDGES);
            GP.LABEL(data.tempLabel); GP.LABEL("","temperature");
          GP.BOX_END();
      GP.BLOCK_END();
...
void portalCheckForm(){
  if (portal.form()) {    
    ...
    // Preferences
      ...
      portal.copyStr("tempLabel", data.tempLabel);
      portal.copyStr("device_name", data.device_name);
      portal.copyInt("theme", data.theme);
      portal.copyInt("sensorRefreshTime", data.sensorRefreshTime);
      memory.updateNow();
      sensorSetTimer();
